...
.topbar{display:flex;align-items:center;padding:0 0.6vmin 1.4vmin}
...
fetch('/status').then(x=>x.json()).then(d=>{
  ...
  if(d.last_result_label){
    ...
    reason.innerHTML = d.last_result_reason ? ('“'+d.last_result_reason+'”<span class="by">— Philadelphia recycling rules</span>') : '';
    return;
  }
  // idle
  setCard('idle');
